...
async def request_otp(payload: schemas.RequestOTPIn, db: AsyncSession = Depends(get_db)):
    ...
    db.add(otp_obj)
    await db.commit()
    # Send via SMS provider in production. For dev:
...
async def verify_otp(payload: schemas.VerifyOTPIn, db: AsyncSession = Depends(get_db)):
    ...
    if not user:
        ...
        db.add(user)
        await db.flush()
        # assign role if provided otherwise default donor
        ...
        # optionally assign role if payload.role provided and not already present
        if payload.role:
            # check existing
            q3 = select(models.user_roles_table).where(models.user_roles_table.c.user_id == user.id, models.user_roles_table.c.role == payload.role.value)
            r3 = await db.execute(q3)
            if not r3.fetchone():
                ins = insert(models.user_roles_table).values(user_id=user.id, role=payload.role.value)
                await db.execute(ins)
    await db.commit()
